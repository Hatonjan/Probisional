...
and look over the code, starting with `sketch.js` to get an overview
...
4. Look at how the rectangle representing the proportion of female
   ...
   - The ratio of female:male staff at Indiegogo is 50:50. Make sure
     that this is correctly visualized on the plot.
...
1. Using the `mapTemperatureToColor()` method set the `fill()` in the
...
4. Test that when selecting a company name from the list the correct
   data is visualized on the canvas and the correct title is
   generated.

#### Pay gap by job 2017 [1]
